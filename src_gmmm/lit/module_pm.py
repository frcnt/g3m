...
from ..model.time import PerModalityTimeSampler
...
class LitModule(pl.LightningModule):
    def __init__(
        ...
        time_sampler: PerModalityTimeSampler,
        ...
        self.save_hyperparameters(ignore=["model", "time_sampler", "metrics"])

    def basic_step(self, batch) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:

        t = self.time_sampler(batch, device=self.device)
